...
const copyHtml: any = ref("");
// single 单步滚动的定时器
let singleWaitTime: any = null;
// move动画的animationFrame定时器
let reqFrame: any = null;
let startPos: any = null;
//记录touchStart时候的posY
let startPosY: any = null;
//记录touchStart时候的posX
let startPosX: any = null;
// mouseenter mouseleave 控制scrollMove()的开关
let isHover: any = false;
let ease: any = "ease-in";
...
function scrollCancle() {
  cancelAnimationFrame(reqFrame as any);
}
...
function scrollInitMove() {
  nextTick(() => {
    ...
    copyHtml.value = "";
    if (unref(isHorizontal)) {
      height.value = unref(wrap as any).offsetHeight;
      width.value = unref(wrap as any).offsetWidth;
      let slotListWidth = unref(slotList as any).offsetWidth;
      // 水平滚动设置warp width
      if (unref(autoPlay)) {
        // 修正offsetWidth四舍五入
        slotListWidth = slotListWidth * 2 + 1;
      }
      unref(realBox as any).style.width = slotListWidth + "px";
    ...
    if (unref(scrollSwitch)) {
      let timer;
      if (timer) clearTimeout(timer);
      copyHtml.value = unref(slotList as any).innerHTML;
      setTimeout(() => {
        realBoxHeight.value = unref(realBox)?.offsetHeight as any;
        scrollMove();
      }, 0);
    } else {
      scrollCancle();
